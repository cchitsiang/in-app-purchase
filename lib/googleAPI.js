...
/**
 * Uses Google API and the receipt only requires purchaseToken string to validate
 *
 */
...
var URL_FMT = 'https://www.googleapis.com/androidpublisher/v3/applications/%s/purchases/%s/%s/tokens/%s%s';
...
function validatePurchase(_googleServiceAccount, receipt, cb) {
    ...
    _getToken(googleServiceAccount.clientEmail, googleServiceAccount.privateKey, function (error, token) {
        ...
        request({
            method: 'GET',
            url: _getValidationUrl(receipt) + '?access_token=' + token,
            json: true
            ...
            new Promise((resolve, reject) => {
                if (!body.acknowledgementState) {
                    request({
                        method: 'POST',
                        json: true,
                        url: _getValidationUrl(receipt) + ':acknowledge',
                        body: receipt.developerPayload,
                        headers: {
...
function shouldAcknowledge(receipt) {
...
function _getValidationUrl(receipt) {
    return util.format(
        ...
        encodeURIComponent(receipt.subscription === true ? 'subscriptions' : 'products'),
        encodeURIComponent(receipt.productId),
        encodeURIComponent(receipt.purchaseToken),
        encodeURIComponent(shouldAcknowledge(receipt) ? ':acknowledge' : '')
    );
}
